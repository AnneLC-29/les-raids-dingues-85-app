...
# Sécurisation des colonnes BDD
for col in ["Nom de la course", "Date", "Lieu", "Type de course", "Détail", "Lien", "Visuel", "Lien_Image"]:
    if col not in df_courses.columns:
        df_courses[col] = ""

df_courses = df_courses.dropna(subset=["Nom de la course"]).copy()
df_courses['Nom de la course'] = df_courses['Nom de la course'].astype(str).str.strip()
...
# Sécurisation des colonnes PARTICIPATIONS
for col in ["Nom_Membre", "Nom_Course", "Distance", "Statut", "Resultat", "Date"]:
    if col not in df_participations.columns:
        df_participations[col] = ""
...
def get_coords_smart(lieu_str):
    ...
    if '?' in lieu_clean or '#' in lieu_clean or len(lieu_clean) < 3:
        return None, None
...
with tab_fiche:
    try:
        ...
        if len(liste_courses_labels) == 0:
            ...
            if course_choisie:
                infos_df = df_courses[df_courses["Label_Unique"] == course_choisie]
                if not infos_df.empty:
    ...
    try:
        ...
        with col_details:
            ...
            if map_data and isinstance(map_data, dict) and map_data.get("last_object_clicked_tooltip"):
                course_cliquee_label = str(map_data["last_object_clicked_tooltip"])
            ...
            if not course_cliquee_label:
                ...
                if infos_cliquee_df.empty:
                    c_name_clean = course_cliquee_label.rsplit(" (", 1)[0].strip()
                    infos_cliquee_df = df_courses[df_courses["Nom de la course"] == c_name_clean]
