...
# Normalisation des chaînes & Clé sans ponctuation (Sécurisé PyArrow)
df_courses = df_courses.dropna(subset=["Nom de la course"]).copy()
df_courses['Nom de la course'] = df_courses['Nom de la course'].fillna("").astype(str)
df_courses['Date'] = df_courses['Date'].fillna("").astype(str)
df_courses['Course_Key'] = df_courses['Nom de la course'].apply(clean_course_key)

df_participations['Nom_Course'] = df_participations['Nom_Course'].fillna("").astype(str)
df_participations['Nom_Membre'] = df_participations['Nom_Membre'].fillna("").astype(str)
    ...
    try:
        ...
        if course_choisie:
            ...
            with col1:
                ...
                if pd.notna(infos.get('Lien', '')) and str(infos.get('Lien', '')) != "Clos":
            ...
            inscrits = df_participations[df_participations["Course_Key"] == c_key_choisie].fillna("")
        ...
        st.error(f"Erreur d'affichage - Fiche : {e}\n{traceback.format_exc()}")
...
<table class="cal-table"><thead><tr><th class="cal-th">LUNDI</th><th class="cal-th">MARDI</th><th class="cal-th">MERCREDI</th><th class="cal-th">JEUDI</th><th class="cal-th">VENDREDI</th><th class="cal-th">SAMEDI</th><th class="cal-th">DIMANCHE</th></tr></thead><tbody>"""
            ...
            for day in week:
                if day == 0: html_code += '<td class="cal-td cal-empty"></td>'
                    ...
                    for _, row in courses_jour.iterrows():
                        ...
                        inscrits_df = df_participations[df_participations['Course_Key'] == str(row['Course_Key'])]
                        inscrits_liste = inscrits_df['Nom_Membre'].dropna().tolist()
                        ...
                        # Protection contre les NaN dans 'Détail'
                        detail_course = html.escape(str(row.get('Détail', ''))) if pd.notna(row.get('Détail')) else ""
                        tooltip_body = f"<b>{nom_c}</b><br>📍 {html.escape(str(row['Lieu']))}<br>🏃 {html.escape(str(row['Type de course']))} ({detail_course}){inscrits_html}<br><br><span style='color: #38bdf8; font-weight: bold;'>👉 Clic pour m'inscrire</span>"
                        
                        # Protection Javascript (Apostrophes)
                        js_safe_name = nom_raw.replace("'", "\\'").replace('"', '&quot;')
                        click_action = f"navToCourse('{js_safe_name}')"
        ...
        st.error(f"Erreur d'affichage - Calendrier : {e}\n{traceback.format_exc()}")
    ...
    try:
        ...
        with col_map:
            ...
            for _, row in df_courses.iterrows():
                ...
                if lat and lon:
                    ...
                    detail_c = html.escape(str(row.get('Détail', ''))) if pd.notna(row.get('Détail')) else ""
                    popup_html = f"<div style='font-family: sans-serif; width: 180px;'><b>{html.escape(nom_c)}</b><br>📅 {row['Date']}<br>📍 {row['Lieu']}<br>🏃 {row['Type de course']}<br><small>{detail_c}</small><br><b>👥 Inscrits : {nb_inscrits}</b></div>"
        ...
        with col_details:
            ...
            if map_data and map_data.get("last_object_clicked_tooltip"):
                ...
                course_cliquee = str(clicked_tooltip).rsplit(" (", 1)[0].strip()
            ...
            if not course_cliquee:
                ...
                inscrits_course = df_participations[df_participations["Course_Key"] == c_key].fillna("")
                ...
                if inscrits_course.empty:
                    ...
                    st.dataframe(inscrits_course[disp_cols_c], hide_index=True)
    except Exception as e:
        ...
        if not membres_dispos: st.info("Aucun membre disponible.")
            ...
            if membre_choisi:
                ...
                if p_membre.empty: st.info(f"{membre_choisi} n'a aucune inscription.")
                    ...
                    details_membre = p_membre.merge(df_courses[["Course_Key", "Type de course"]].drop_duplicates(), on="Course_Key", how="left").fillna("")
                    disp_cols_m = [c for c in ["Date", "Nom_Course", "Lieu", "Type de course", "Distance", "Statut", "Resultat"] if c in details_membre.columns]
                    st.dataframe(details_membre[disp_cols_m], hide_index=True)
    except Exception as e:
        st.error(f"Erreur d'affichage - Membre : {e}\n{traceback.format_exc()}")
...
with tab_stats_km:
    try:
        ...
        if df_finishers.empty: 
            st.info("Aucun membre n'a encore le statut 'Terminé' ou 'Finisher'.")
        else:
            ...
            # Application du style en toute sécurité
            def style_rows(row):
                color = 'background-color: #fef9c3; color: #1e293b; font-weight: 500;' if 'Femme' in str(row['Sexe']) else ''
                return [color] * len(row)
                
            styled_df = stats_display.style.apply(style_rows, axis=1).format({'Distance Totale (km)': '{:.1f} km'})
            st.dataframe(styled_df, hide_index=True)
    except Exception as e:
        st.error(f"Erreur d'affichage - Classement KM : {e}\n{traceback.format_exc()}")
    ...
    try:
        ...
        if df_courses_a_date.empty:
            ...
            with col_s_left:
                ...
                st.dataframe(df_month_stats_total, hide_index=True)
                st.write("")
                ...
                with col_d_title: st.write("#### 🗺️ Départements parcourus")
                with col_d_metric: st.metric("Départements distincts", len(df_depts))
                st.dataframe(df_depts, hide_index=True)
            ...
            with col_s_right:
                ...
                with col_p1: st.metric("Total participants à date", total_participants)
                with col_p2: st.metric("Total km validés (Finishers)", f"{total_km_a_date:.1f} km")
                ...
                if df_participations_a_date.empty:
                    ...
                    df_part_course = df_participations_a_date_temp.groupby("Nom_Course").agg(
                        Nb_Participants=('Nom_Membre', 'count'), Total_Km=('Km_Finisher', 'sum')
                    ).reset_index()
                    
                    df_part_course = df_part_course.sort_values(by="Nb_Participants", ascending=False)
                    df_part_display = df_part_course.rename(columns={'Nom_Course': 'Manifestation', 'Nb_Participants': 'Nombre de participants', 'Total_Km': 'Km réalisés (Finishers)'})[['Manifestation', 'Nombre de participants', 'Km réalisés (Finishers)']]
                    
                    st.dataframe(df_part_display.style.format({'Km réalisés (Finishers)': '{:.1f} km'}), hide_index=True)
    except Exception as e:
        st.error(f"Erreur d'affichage - Statistiques : {e}\n{traceback.format_exc()}")